...
stDate ConvertDayesToDate(stDate &Date)
{

    short RemainingOfDays = NumberOfDaysFromTheBeginingOfTheYear(Date.Year, Date.Month, Date.Day);
    short MonthDays=0;
    ...
    while(true)
    {
        ...
        if(MonthDays<RemainingOfDays)
        {
            Date.Month++;
            RemainingOfDays -= MonthDays;
        }
        else
        {
            Date.Day = RemainingOfDays;
            break;
        }
    }
    return Date;
...
int main()
{
    ...
    cout << "Number Of Days From The Beginning Of The Year is : " << NumberOfDaysFromTheBeginingOfTheYear(Date.Year, Date.Month, Date.Day) << endl;
    
    ConvertDayesToDate(Date);
