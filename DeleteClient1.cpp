...
bool MarkClientForDeleteByAccountNumber(vector <stClient> &vClient, string AccountNumber)
{
    for(stClient &C : vClient)
    {
        if(C.AccountNumber == AccountNumber)
...
void DeleteClientByAccountNumber(vector <stClient> vClient, string AccountNumber ,string ClientsFileName)
{
    char Answer = 'Y';
    stClient Client;
...
int main()
{
    ...
    DeleteClientByAccountNumber(vClient, AccountNumber, FileName);
}